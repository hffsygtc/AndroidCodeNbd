...
public abstract class PlatformPageAdapter extends ViewPagerAdapter implements OnClickListener {
	...
	public static final int DESIGN_BOTTOM_HEIGHT = 70;
...
//		llPanel.setBackgroundColor(0xfff2f2f2);
		llPanel.setBackgroundColor(0xfff4f5f6);

		int lineCount = panelHeight / cellHeight;
		Log.e("SHARE_PANEL", "line count==>"+lineCount+"line size==>"+lineSize);
...
//				llCells[i * lineSize + j].setBackgroundResource(cellBack);
				llCells[i * lineSize + j].setBackgroundColor(0xfff4f5f6);
...
//			ivLogo.setScaleType(ScaleType.CENTER_INSIDE);
			...
			lp = new LinearLayout.LayoutParams(logoHeight, logoHeight);
//			lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, logoHeight);
			lp.topMargin = paddingTop;
			//正常情况下没有leftpadding 应该注释
			lp.leftMargin = paddingTop;
			llCell.addView(ivLogo, lp);

			TextView tvName = new TextView(context);
			tvName.setTextColor(0xff010101);
//			tvName.setTextSize(TypedValue.COMPLEX_UNIT_SP, 9);
			tvName.setTextSize(TypedValue.COMPLEX_UNIT_PX, 27);
...
//				llCells[i].setBackgroundResource(disableBack);
				llCells[i].setBackgroundColor(0xfff4f5f6);
