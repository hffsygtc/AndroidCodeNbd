...
public abstract class LoadingRenderer {
	...
	private final ValueAnimator.AnimatorUpdateListener mAnimatorUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
		...
		public void onAnimationUpdate(ValueAnimator animation) {
			computeRender(Float.parseFloat(animation.getAnimatedValue().toString()));
			invalidateSelf();
		}
	};

	/**
	 * Whenever {@link LoadingDrawable} boundary changes mBounds will be
	 * updated. More details you can see
	 * {@link LoadingDrawable#onBoundsChange(Rect)}
	 */
	...
	void stop() {
		...
		// it will always call the method onAnimationUpdate(ValueAnimator
		// animation)
		// why ? if you know why please send email to me
		// (dinus_developer@163.com)
	...
	private void initParams(Context context) {
		mWidth = SizeUtils.dp2px(context, DEFAULT_SIZE);
		mHeight = SizeUtils.dp2px(context, DEFAULT_SIZE);
	...
	private void setupAnimators() {
		...
		mRenderAnimator.setDuration(mDuration);
		// fuck you! the default interpolator is
